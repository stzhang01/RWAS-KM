...
//			System.out.println(line);
			...
			if ((tmp_arr[0].equals(Gene_Chromosome)) && (pos>=(Gene_ENSG_Start) ) 
					&& (pos<=(Gene_ENSG_End )  )) {
				String rs=  "rs_"+Integer.toString(snp_index);
				String ref = SNP_REF;
				String alt = SNP_ALT;
...
//				}
	...
	public static void get_result(String out_folder) 
		...
		BufferedReader br_info = new BufferedReader(new FileReader(out_folder+"RWAS-KM.result"));
		while ((line = br_info.readLine()) != null) {
			 line= line.replace("\n", "").replace("\r", "");
			 String[] tmp_arr = line.split("\t");
...
//		System.out.println("-------------------------------------");
		System.out.println("The p-value of RWAS-KM on regulatory peak "+ Gene_ENSG_ID+":\t"+ 
...
//        bw.write(ss+"\n");
        ...
        bw.write(ss+"\n");
        ss="write(ss ,\""+out_folder+"RWAS-KM.result\")";
        bw.write(ss+"\n");
        bw.close();
	}
...
//					ss_eqtl= rs_dict.get(pos)+"\t1.0";
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		System.out.println("Welcome to RWAS-KM, your gateway to genetic insights with "
				+ "Regulome-Wide Association Studies enhanced by Kernel Machine!");

		if(args.length==0){
			
			System.out.println("RWAS-KM: integrating Kernel Machine with Regulome-Wide Association Studies, "
					+ "unlocking key insights into the genetic basis of cancers.\n Usage:\nRWAS-KM\n"
					+ "-format csv|plink\n"
			...
			if(args.length < 14){
				...
				for(int k=1;k<args.length;k++){
					if(args[k].startsWith("-")){
						...
						else if(args[k].equals("-weights_info")) info_file=args[k+1];
				...
				if(genotype_file==null){
					System.out.println("Kindly provide the path to your genotype file!");
					System.exit(0);
				} if(format==null){
					System.out.println("Please specify the genotype file format: CSV or PLINK!");
					System.exit(0);
				}else if (phenotype_file==null){
					System.out.println("Please input the phenotype file!");
					System.exit(0);
				} else if (column==null){
					System.out.println("Please provide the path to your phenotype file!");
					System.exit(0);
				} else if (info_file==null){
					System.out.println("Please provide the path for the rQTL information file.");
					System.exit(0);
				} else if (output_folder==null){
					System.out.println("Please specify the path for the output folder where results will be saved!");
					System.exit(0);
				} else if (rscript==null){
					System.out.println("Provide the absolute path to the Rscript binary file.");
					System.exit(0);
				} else if (gene_id==null){
					System.out.println("Please provide regulatory peak id!");
					System.exit(0);
				}else if (plink==null){
					System.out.println("Kindly provide the path to the plink binary file!");
					System.exit(0);
				}else if (phenotype_type==null){
					System.out.println("Please specify the phenotype type: continuous or binary!");
					System.exit(0);
				}
				...
				if (format.equals("csv")) {
					System.out.println("Initiating selection and conversion of CSV file to PLINK format... ...\t"+
					...
					select_plink( genotype_file,  phenotype_file, 
							  output_folder, Integer.parseInt(column)) ;
					System.out.println("Commencing selection of variants located within the regulatory peak region.\t"+
							dtf.format(LocalDateTime.now()));
				} else {
					System.out.println("Ensure the input genotype file is in either csv or plink format!");
					System.exit(0);
				}
				
				System.out.println("Selection and conversion of the CSV file to PLINK format "
						+ "has been successfully completed.\t"+
						dtf.format(LocalDateTime.now()));
				System.out.println("=====================================");
				
				System.out.println("Initiating the creation of BED files and recoding of the PLINK files.\t"+
						dtf.format(LocalDateTime.now()));
				
				make_bed(output_folder, plink);		
				recode(output_folder, plink);
				System.out.println("Creation of BED files and recoding of the "
						+ "PLINK files have been successfully completed.\t"+
						dtf.format(LocalDateTime.now()));
				
				System.out.println("=====================================");
	
				System.out.println("Executing SKAT (Sequence Kernel Association Test) with rQTL weights...\t"+
						dtf.format(LocalDateTime.now()));
				write_R(output_folder, phenotype_type);
				run_Skat(output_folder, rscript);
				System.out.println("Execution of SKAT with rQTL weights has been successfully completed.\t"+
						dtf.format(LocalDateTime.now()));
				System.out.println("=====================================");
